...
async function focusExistingWindow(windowId) {
  ...
  // Save the current window's tabs as a workspace.
  async function saveWorkspace(name) {
    try {
      log(`Starting saveWorkspace for workspace: "${name}"`);
      const currentWindow = await browser.windows.getCurrent();
      log("Current window retrieved:", currentWindow);
      const tabs = await browser.tabs.query({ currentWindow: true });
      log("Tabs retrieved in current window:", tabs);
      const urls = tabs.map(tab => tab.url);
      const workspaceData = {
        windowId: currentWindow.id,
        tabs: urls
      };
      log("Saving workspace data:", workspaceData);
      await browser.storage.local.set({ [name]: workspaceData });
      log(`Workspace "${name}" saved successfully.`);
      return { status: "ok" };
    } catch (error) {
      return handleError(error, "saveWorkspace");
    }
  }
