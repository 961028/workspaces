...
async function handleGetState(sendResponse) {
  try {
    ...
    let openWindowIds = openWindows.map(win => win.id);
    ...
    let unsavedWindows = openWindows.filter(win => !savedWindowIds.includes(win.id))
      .map(win => {
        ...
        return { windowId: win.id, title: label };
      });
...
browser.windows.onCreated.addListener(window => {
  // Newly created windows will be available in the popup unsaved list.
  console.info("Window created", window.id);
});

browser.windows.onRemoved.addListener(windowId => {
  // Mark any workspace using the closed window as no longer open.
