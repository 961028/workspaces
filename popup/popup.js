...
class PopupApp {
	...
	async loadState() {
		try {
			...
			if (!currentWindow || !currentWindow.id) {
				...
				this.statusBar.show(
					"Failed to retrieve window information.",
					true,
				);
				return;
			}
			...
			if (response && response.success) {
				this.workspaceList.updateSavedList(
					response.saved,
					currentWindowId,
				);
				this.workspaceList.updateUnsavedList(
					response.unsaved,
					currentWindowId,
				);
			} else {
				this.statusBar.show(
					response?.error || "Failed to retrieve state.",
					true,
				);
			}
		} catch (err) {
	...
	async sendMessage(message) {
		...
		try {
			...
			this.statusBar.show(
				error.message || "Communication error with background script.",
				true,
			);
		}
	}
	...
	persistSavedOrder() {
		try {
			...
			if (!savedList) {
				console.error(
					"Cannot persist order; saved list element not found.",
				);
				this.statusBar.show("Failed to persist order.", true);
				return;
			}
			const order = Array.from(
				savedList.querySelectorAll("li.saved-item"),
			).map((item) => parseInt(item.dataset.wsid, 10));
...
class WorkspaceList {
	...
	updateSavedList(saved, currentWindowId) {
		...
		if (!Array.isArray(saved) || saved.length === 0) {
			list.innerHTML =
				'<div class="empty-message">You haven\'t saved any windows yet.</div>';
			return;
		}
    ...
    `;
		li.querySelector(".label").textContent =
			workspace.title || "(No Title)";
		li.querySelector(".subtitle").textContent = subtitle;
		this.setFavicon(
			li,
			workspace.windowId,
			workspace.favicon || DEFAULT_FAVICON,
		);
	...
	createUnsavedListItem(win, currentWindowId) {
		...
		this.dragAndDropManager.addListItemEvents(li, {
			...
			onClick: () => {
				this.sendMessage?.({
					action: "focusWindow",
					windowId: parseInt(win.windowId, 10),
				});
			},
...
class ContextMenu {
	...
	create() {
		try {
			...
			const unsaveItem = this.createContextMenuItem(
				"Unsave",
				"context-menu-item",
				() => this.onUnsaveClick(),
			);
...
class DragAndDropManager {
	...
	pointerDownHandler = (e) => {
		...
		this.draggableItem.addEventListener(
			"pointermove",
	...
	initItemsState() {
		...
		const idleItems = allItems.filter((item) =>
		...
		idleItems.forEach((item) => {
			if (this.isItemToggled(item)) {
				...
				this.setItemTransform(
					item,
					direction * (draggableRect.height + ITEMS_GAP),
				);
			} else {
	...
	removePointerListeners() {
		if (!this.draggableItem) return;
		this.draggableItem.removeEventListener(
			"pointermove",
			this.pointerMoveHandler,
		);
		this.draggableItem.removeEventListener(
		...
		browser.theme.onUpdated.addListener(async ({theme, windowId}) => {
			try {
				...
				if (!windowId || windowId === currentWindow.id) {
					...
					console.info(
						"Theme update skipped for windowId:",
						windowId,
					);
				}
			} catch (error) {
...
function createPopupApp() {
	...
	dragAndDropManager.setPersistSavedOrderHandler(
		app.persistSavedOrder.bind(app),
	);
