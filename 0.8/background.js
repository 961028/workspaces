 ...
 * The workspace title is updated with the active tab's title only if the workspace
 * was not manually renamed (i.e. no user-defined title exists).
 *
...
function updateWorkspaceForWindow(workspaces, winId, tabs) {
  // Find the active tab in the window; if not found, use the last tab.
  const activeTab = tabs.find(tab => tab.active) || tabs[tabs.length - 1];
  Object.keys(workspaces).forEach(wsId => {
    let ws = workspaces[wsId];
    if (ws.windowId === winId) {
      // Always update the list of tab URLs.
      ws.tabs = tabs.map(tab => tab.url);
      // Only auto-update the title if the workspace hasn't been customized by the user.
      if (!ws.userDefined) {
        ws.title = activeTab ? activeTab.title : "";
      }
    }
  });
}
...
async function handleSaveWindow(windowId, sendResponse) {
  try {
    ...
    // Create a new workspace. The workspace is not user-defined by default.
    let newWorkspace = {
      id: nextId,
      windowId: windowId,
...
/**
 * Renames a saved workspace without being overridden by tab events.
 *
...
async function handleRenameWorkspace(workspaceId, newTitle, sendResponse) {
  try {
    ...
    // Set the new title and mark the workspace as user-defined.
    workspaces[workspaceId].title = newTitle;
    workspaces[workspaceId].userDefined = true;
    await browser.storage.local.set({ workspaces, nextId });
...
/**
 * Unsaves a workspace (removes it from storage).
 *
...
async function handleUnsaveWorkspace(workspaceId, sendResponse) {
  try {
    ...
    await browser.storage.local.set({ workspaces, nextId });
    console.info("Unsaved workspace", workspaceId);
    sendResponse({ success: true });
  } catch (e) {
...
browser.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  // Indicate that we'll send the response asynchronously.
  return true;
});
